...
class MontagemWindow(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        ttk.Label(rotation_frame, text="Rotação:", font=("Arial", 8, "bold")).pack(side=LEFT)
        ...
        rotation_scale.pack(side=LEFT, fill=X, expand=True, padx=5)
        
        self.rotation_label = ttk.Label(rotation_frame, text="0°", width=5)
        self.rotation_label.pack(side=RIGHT)
        
        # Botão para resetar rotação
        ttk.Button(rotation_frame, text="Reset", width=6,
                  command=self.reset_rotation).pack(side=RIGHT, padx=(0, 5))
    ...
    def find_slot_at(self, canvas_x, canvas_y):
        """Encontra slot nas coordenadas do canvas, incluindo slots rotacionados."""
        for slot in self.slots:
            ...
            # Verifica rotação do slot
            rotation = slot.get('rotation', 0)
            
            if rotation == 0:
                # Slot não rotacionado - verificação simples
                if x1 <= canvas_x <= x2 and y1 <= canvas_y <= y2:
                    return slot
            else:
                # Slot rotacionado - verificação usando transformação inversa
                import math
                
                # Calcula centro do slot
                center_x = (x1 + x2) / 2
                center_y = (y1 + y2) / 2
                
                # Translada ponto para origem
                px = canvas_x - center_x
                py = canvas_y - center_y
                
                # Aplica rotação inversa
                rad = math.radians(-rotation)  # Rotação inversa
                cos_r = math.cos(rad)
                sin_r = math.sin(rad)
                ...
                # Verifica se está dentro do retângulo não rotacionado
                w_half = (x2 - x1) / 2
                h_half = (y2 - y1) / 2
                
                if -w_half <= rx <= w_half and -h_half <= ry <= h_half:
                    return slot
    ...
    def add_slot(self, xa, ya, wa, ha):
        ...
        slot_data = {
            ...
            'rotation': 0,   # Rotação em graus (valor fixo em 0)
...
"""
    ...
    def on_rotation_change(self, value):
        """Callback para mudança no controle de rotação."""
        try:
            rotation = float(value)
            self.rotation_label.config(text=f"{rotation:.0f}°")
            
            # Se há um slot selecionado, aplica a rotação
            if hasattr(self, 'selected_slot_id') and self.selected_slot_id is not None:
                for slot in self.slots:
                    if slot['id'] == self.selected_slot_id:
    ...
    def reset_rotation(self):
        ...
        # Se há um slot selecionado, reseta sua rotação
        if hasattr(self, 'selected_slot_id') and self.selected_slot_id is not None:
            for slot in self.slots:
                if slot['id'] == self.selected_slot_id:
                    slot['rotation'] = 0.0
